...
/*
要求:我们摸拟注册操作,如果用户名已存在，则抛出异常并提示:亲，该用户名已经被注册。
分析:
1.快用数组保存已经注册过的用户名(數据库)
...
public class Demo12RegisterException {
    ...
    //3.定义一个方法,对用户输入的中注册的用户名进行判断
    private static void checkUsername(String username) /*throws RegisterFailedException*/ {
        //遍历存储已经注册过用户名的数组,获歌每一个用户名
        for (String name : usernames) {
            //使用获取到的用户名和用户输入的用户名比较
            if(name.equals(username)){
                //true用户名已经存在,抛出RegisterException异常,告知用户"亲，该用户名已经被注册”;
